...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_tunnel_tracker.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_tunnel_tracker"
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_tunnel_tracker'
    run_command(context, command)
...
def check_migrations(context):
    ...
    command = "nautobot-server --config=/opt/nautobot/nautobot_config.py makemigrations --dry-run --check"
...
def unittest(context, keepdb=False, label="nautobot_tunnel_tracker", failfast=False, buffer=True):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_tunnel_tracker/*' --omit *migrations*"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.nautobot_tunnel_tracker.local):
